...
void processSeeds(vector<vector<SeedMap>>& seedMaps, vector<unsigned long long>& seeds) {
    unsigned long long seedMin = -1, seedLoc, rangeStart, rangeEnd;
...
void numComb(vector<unsigned long long>& seeds) {
    unsigned long long seedMin = -1, seedLoc = 0, rangeStart, rangeEnd;
    for (int i = 0; i < seeds.size(); i += 2) {
        rangeStart = seeds[i];
        seedLoc += seeds[i+1];
    }
